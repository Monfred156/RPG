...
void find_two_direction_tuto(global *gb, float time_sec, int choose, int sprite)
{
    switch (choose) {
        case MOVE_TOP_L:
            movement_top_player_tuto(gb, time_sec, 0, sprite);
            movement_left_player_tuto(gb, time_sec, sprite);
            break;
        case MOVE_BACK_L:
            movement_back_player_tuto(gb, time_sec, 0, sprite);
            movement_left_player_tuto(gb, time_sec, sprite);
            break;
        case MOVE_TOP_R:
            movement_top_player_tuto(gb, time_sec, 0, sprite);
            movement_right_player_tuto(gb, time_sec, sprite);
            break;
        case MOVE_BACK_R:
            movement_back_player_tuto(gb, time_sec, 0, sprite);
            movement_right_player_tuto(gb, time_sec, sprite);
...
void find_one_direction_tuto(global *gb, float time_sec, int choose, int sprite)
{
    switch (choose) {
        case MOVE_TOP:
            movement_top_player_tuto(gb, time_sec, 1, sprite);
            break;
        case MOVE_BACK:
            movement_back_player_tuto(gb, time_sec, 1, sprite);
            break;
        case MOVE_LEFT:
            movement_left_player_tuto(gb, time_sec, sprite);
            break;
        case MOVE_RIGHT:
            ...
            break;
        default:
            find_two_direction_tuto(gb, time_sec * 0.8, choose, sprite);
    }
}
...
int count_direction_anim_tuto(global *gb, int sprite, float time_sec)
{
    ...
    if (choose == 3 || choose == 7 || choose == 11)
        choose -= 3;
    find_one_direction_tuto(gb, time_sec, choose, sprite);
    return (choose);
}
...
void event_move_player_tuto(global *gb, int sprite)
{
    ...
    if (count_direction_anim_tuto(gb, sprite, time_sec) == 0)
        gb->move->walk = 0;
    if (gb->move->walk >= 8)
        gb->move->walk = 1;
