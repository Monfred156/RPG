...
{
    if (gb->inv.inv[*place] >= AXE &&
    gb->inv.inv[*place] <= LANCE && gb->inv.inv[18] == 0) {
        gb->inv.inv[18] = gb->inv.inv[*place];
        increase_stats_weapon(gb);
        gb->inv.inv[*place] = 0;
    }
    if ((gb->inv.inv[*place] == ARMOR || gb->inv.inv[*place] == ARMOR2) &&
    gb->inv.inv[17] == 0) {
        gb->inv.inv[17] = gb->inv.inv[*place];
        increase_stats_armor(gb);
        gb->inv.inv[*place] = 0;
    }
}
