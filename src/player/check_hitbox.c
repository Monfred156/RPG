...
*limit_value)
{
    ...
    for (int i = limit_value[0]; i <= limit_value[1]; i++) {
        ...
        gb->hitbox[i].hitbox);
        if (hero.top >= hitbox.top &&
        hero.top <= hitbox.top + hitbox.height &&
        hero.left + hero.width >= hitbox.left &&
        hero.left <= hitbox.left + hitbox.width)
            return 1;
    }
    return 0;
}
...
int *limit_value)
{
    ...
    for (int i = limit_value[0]; i <= limit_value[1]; i++) {
        sfFloatRect hitbox = sfRectangleShape_getGlobalBounds(gb->hitbox[i]
                .hitbox);
        if (hero.left + hero.width >= hitbox.left &&
        hero.left + hero.width <= hitbox.left + hitbox.width
        && hero.top + hero.height >= hitbox.top &&
        hero.top <= hitbox.top + hitbox.height)
            return 1;
    }
    return 0;
...
int *limit_value)
{
    ...
    for (int i = limit_value[0]; i <= limit_value[1]; i++) {
        sfFloatRect hitbox = sfRectangleShape_getGlobalBounds(
        gb->hitbox[i].hitbox);
        if (hero.top + hero.height >= hitbox.top &&
        hero.top + hero.height <= hitbox.top + hitbox.height &&
        hero.left + hero.width >= hitbox.left &&
        hero.left <= hitbox.left + hitbox.width)
            return 1;
    }
